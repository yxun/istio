...
func (validator *Validator) Run() error {
	log.Infof("Starting nftables validation. This check verifies that nftables rules are properly established for the network.")
	s := Service{
		validator.Config,
	}
	...
	case err := <-sError:
		if err == nil {
			log.Info("Validation passed, nftables rules established")
		} else {
...
// TODO(lambdai): remove this if nftables only need to redirect to outbound proxy port on A call A
...
func NewValidator(config *config.Config) *Validator {
	...
	return &Validator{
		Config: &Config{
			...
			ServerOriginalPort:  config.NftablesProbePort,
