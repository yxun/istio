...
const InvalidDropByNftables = "INVALID_DROP"
...
func bindCmdlineFlags(cfg *config.Config, cmd *cobra.Command) {
	...
	flag.BindEnv(fs, constants.DryRun, "n", "Do not call any external dependencies like nft.",
		&cfg.DryRun)

	flag.BindEnv(fs, constants.NftablesProbePort, "", "Set listen port for failure detection.", &cfg.NftablesProbePort)

	flag.BindEnv(fs, constants.ProbeTimeout, "", "Failure detection timeout.", &cfg.ProbeTimeout)

	flag.BindEnv(fs, constants.SkipRuleApply, "", "Skip nftables apply.", &cfg.SkipRuleApply)

	flag.BindEnv(fs, constants.RunValidation, "", "Validate nftables.", &cfg.RunValidation)
	...
	flag.BindEnv(fs, constants.DropInvalid, "", "Enable invalid drop in the nftables rules.", &cfg.DropInvalid)
	// This could have just used the default but for backwards compat we support the old env.
	flag.AdditionalEnv(fs, constants.DropInvalid, InvalidDropByNftables)
	...
	flag.BindEnv(fs, constants.NetworkNamespace, "", "The network namespace that nftables rules should be applied to.",
		&cfg.NetworkNamespace)

	flag.BindEnv(fs, constants.CNIMode, "", "Whether to run as CNI plugin.", &cfg.HostFilesystemPodNetwork)

	flag.BindEnv(fs, constants.Reconcile, "", "Reconcile pre-existing and incompatible nftables rules instead of failing if drift is detected.",
		&cfg.Reconcile)

	flag.BindEnv(fs, constants.CleanupOnly, "", "Perform a forced cleanup without creating new nftables chains or rules.",
		&cfg.CleanupOnly)

	// This flag is a safety measure in case the idempotency changes of #50328 backfire.
	// Allow bypassing of nftables idempotency handling, and attempts to apply nftables rules regardless of table state, which may cause unrecoverable failures.
	// Consider removing it after several releases with no reported issues.
	flag.BindEnv(fs, constants.ForceApply, "", "Apply nftables changes even if they appear to already be in place.",
		&cfg.ForceApply)
...
func GetCommand(logOpts *log.Options) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "istio-nftables",
		Short: "Set up nftables rules for Istio Sidecar",
		Long:  "istio-nftables is responsible for setting up port forwarding for Istio Sidecar.",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if cfg.RunValidation {
				...
				if err := validator.Run(); err != nil {
					...
					msg := fmt.Errorf(`nftables validation failed; workload is not ready for Istio.
...
Otherwise, this pod will need to be manually removed so that it is scheduled on a node with istio-cni running, allowing nftables rules to be established.
`)
